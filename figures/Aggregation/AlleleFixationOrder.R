...
# Assumes that QtlMonteCarlo and TraitArchitectureFounder have already been run
library(dplyr)
library(ggplot2)
library(ggpubr)
library(grid)
...
# The directory of the 'fixed_alleles.csv' outputs of QtlMonteCarlo.R
setwd("~/Documents/CSU/FitnessLandscapes/output/QtlMonteCarlo/effect_sizes")

# The directory to write the multipanel, and where the 'initial_architecture.csv'
# which was generated by TraitArchitectureFounder.R lives
output_dir <- file.path(getwd(), "../../Geometric/3_15")
if (!dir.exists(output_dir)) dir.create(output_dir)

# QTL: 1
# 50 pop resets x 2 sims (N=100)
qtl1_100 <- read.csv("qtl_1_Ne_1000_var_0.4_2026-02-26_21_53/fixed_alleles.csv")
qtl1_100$qtl <- 1
qtl1_100$var <- 0.4

# 200 pop resets x 2 sims (N=400)
qtl1_400 <- read.csv("qtl_1_Ne_1000_var_0.4_2026-02-27_22_27/fixed_alleles.csv")
qtl1_400$qtl <- 1
qtl1_400$var <- 0.4

# QTL: 2
# 50 pop resets x 2 sims (N=100)
qtl2_100 <- read.csv("qtl_2_Ne_1000_var_0.4_2026-02-27_00_24/fixed_alleles.csv")
qtl2_100$qtl <- 2
qtl2_100$var <- 0.4

# 100 pop resets x 2 sims (N=200)
qtl2_200_a <- read.csv("qtl_2_Ne_1000_var_0.4_2026-02-28_08_42/fixed_alleles.csv")
qtl2_200_a$qtl <- 2
qtl2_200_a$var <- 0.4

# 100 pop resets x 2 sims (N=200)
qtl2_200_b <- read.csv("qtl_2_Ne_1000_var_0.4_2026-03-01_21_25/fixed_alleles.csv")
qtl2_200_b$qtl <- 2
qtl2_200_b$var <- 0.4

# QTL: 5
# 50 pop resets x 2 sims (N=100)
qtl5_100 <- read.csv("qtl_5_Ne_1000_var_0.4_2026-02-27_02_55/fixed_alleles.csv")
qtl5_100$qtl <- 5
qtl5_100$var <- 0.4

# 100 pop resets x 2 sims (N=200)
qtl5_200_a <- read.csv("qtl_5_Ne_1000_var_0.4_2026-02-28_13_52/fixed_alleles.csv")
qtl5_200_a$qtl <- 5
qtl5_200_a$var <- 0.4

# 100 pop resets x 2 sims (N=200)
qtl5_200_b <- read.csv("qtl_5_Ne_1000_var_0.4_2026-03-02_02_39/fixed_alleles.csv")
qtl5_200_b$qtl <- 5
qtl5_200_b$var <- 0.4

# Merge all the data together
fixed_df <- rbind(qtl1_100,
                  qtl1_400,
                  qtl2_100,
                  qtl2_200_a,
                  qtl2_200_b,
